...
See `mpas_hfip_realtime_edited_rocoto.xml` for the Rocoto XML. This section describes the jobs in the workflow.

### Overview

The workflow runs in these stages:

1. Initialization: *get_ics_data* gets GFS data, *ungrib_ics* converts it to WPS format, and *mpas_ics* generates MPAS input files.
2. Forecast: *mpas* tries to run the forecast, and usually hangs. If it hangs, *mpas_restart* takes over, running 24 hours per job execution, as many times as it takes.
3. Post-processing: *mpassit* interpolates MPAS data to a rectangular grid, *upp* generates GRIB products. Other jobs scrub or handle nicites.
4. Products: *tracker* runs the vortex tracker, and *graphics* uses pygraf to make web graphics.
5. Archiving and Scrubbing: Archiving is split across nine separate jobs. Scrubbing is split across four. Dependencies ensure files who should be archived aren't scrubbed first.

### Step-by-Step Details

1. Just after the synoptic time, a cron job submits the *dummy_job_to_force_rocoto_to_start_the_cycle* job to force Rocoto to start the cycle. This job does nothing on its own; the act of running it is enough. (See `mpas_hfip_cron.sh`)
...
3. *ungrib_ics* converts GRIB files to intermediate files usable by MPAS. (Runs in a reservation.)
4. *mpas_ics* generates an MPAS input file from the prior step's files. (Runs in a reservation.)
5. *mpas* attempts to run the MPAS 120 hour forecast, and usually hangs. (Runs in a reservation.)
6. *mpas_restart* only runs if the *mpas* job hangs. Doesn't run in a reservation. It forecasts a maximum of 24 hours before resubmitting. This is a simpler job than *mpas*, reducing the chances of errors from, for example, conflicting library versions from conda.
7. Post-processing stage. Each time model output files diag and history streams are both available for a forecast time, the following happens.
...
8. *tracker* starts after post-processing is complete, running the GFDL Vortex Tracker
9. *graphics* is run at the same time as the tracker, runing pygraf to make web graphics
10. Archiving and Scrubbing - Several of these jobs may run at once. All wait for post-processing to complete, but some may have additional dependencies.
    1. *archive_mpassit* six jobs (one per day) archive MPASSIT output into six archives using htar. (Files are too large to fit in one archive.)
...
11. *final* marks the cycle as complete when all jobs are complete except *mpas* or *mpas_restart*. This is necessary because no cycles will complete both jobs.
